...
/**
 * <!-- START SNIPPET: description --> This interceptor sets all parameters on
 * the value stack.
 *
 * This interceptor gets all parameters from
 * {@link ActionContext#getParameters()} and sets them on the value stack by
 * calling {@link ValueStack#setValue(String, Object)}, typically resulting in
 * the values submitted in a form request being applied to an action in the
 * value stack. Note that the parameter map must contain a String key and often
 * containers a String[] for the value.
 *
 ...
 * assume that one has an action that contains a property named 'modelClass'
 * that allows to choose what is the underlying implementation of model. By
 * assuring that modelClass property is set before any model properties are set,
 * it's possible to choose model implementation during action.setModelClass()
 * call. Similiarily it's possible to use action.setPrimaryKey() property set
 * call to actually load the model class from persistent storage. Without any
 * assumption on parameter order you have to use patterns like 'Preparable'.
 *
 ...
 * done in the {@link #acceptableName(String)} method. In addition to this
 * method, if the action being invoked implements the {@link ParameterNameAware}
 * interface, the action will be consulted to determine if the parameter should
 * be set.
 *
 * In addition to these restrictions, a flag (
 * {@link ReflectionContextState#DENY_METHOD_EXECUTION}) is set such that no
 * methods are allowed to be invoked. That means that any expression such as
 * <i>person.doSomething()</i> or <i>person.getName()</i> will be explicitely
 * forbidden. This is needed to make sure that your application is not exposed
 * to attacks by malicious users.
 *
 * While this interceptor is being invoked, a flag (
 * {@link ReflectionContextState#CREATE_NULL_OBJECTS}) is turned on to ensure
 * that any null reference is automatically created - if possible. See the type
 * conversion documentation and the {@link InstantiatingNullHandler} javadocs
 * for more information.
 *
 * Finally, a third flag ({@link XWorkConverter#REPORT_CONVERSION_ERRORS}) is
 * set that indicates any errors when converting the the values to their final
 * data type (String[] -&gt; int) an unrecoverable error occured. With this flag
 * set, the type conversion errors will be reported in the action context. See
 * the type conversion documentation and the {@link XWorkConverter} javadocs for
 * more information.
 *
 * If you are looking for detailed logging information about your parameters,
 * turn on DEBUG level logging for this interceptor. A detailed log of all the
 * parameter keys and values will be reported.
 *
 * <b>Note:</b> Since XWork 2.0.2, this interceptor extends
 * {@link MethodFilterInterceptor}, therefore being able to deal with
 * excludeMethods / includeMethods parameters. See [Workflow Interceptor] (class
 * {@link DefaultWorkflowInterceptor}) for documentation and examples on how to
 * use this feature. <!-- END SNIPPET: description -->
 *
 ...
 * </li>
 * <li>acceptParamNames - a comma delimited list of regular expressions to
 * describe a whitelist of accepted parameter names. Don't change the default
 * unless you know what you are doing in terms of security implications</li>
 * <li>excludeParams - a comma delimited list of regular expressions to describe
 * a blacklist of not allowed parameter names</li>
 * <li>paramNameMaxLength - the maximum length of parameter names; parameters
 * with longer names will be ignored; the default is 100 characters</li>
 * </ul>
 *
 ...
 * {@link ParameterNameAware} interface in your actions. However, if you wish to
 * apply a global rule that isn't implemented in your action, then you could
 * extend this interceptor and override the {@link #acceptableName(String)}
 * method.
 *
 * <!-- END SNIPPET: extending -->
 *
 *
 * <!-- START SNIPPET: extending-warning --> Using {@link ParameterNameAware}
 * could be dangerous as @{link
 ...
 * @{link ParameterNameAware#acceptableParameterName(String)} <!-- END SNIPPET:
 *        extending-warning -->
 *
 *
 *        <u>Example code:</u>
...
public class ParametersInterceptor extends MethodFilterInterceptor {
	 ...
	 * that are allowed in the parameter map (aka whitelist).
	 * <p/>
	 ...
	 *            <p/>
	 *            In this class this is a no-op, since the parameters were
	 *            fetched from the same location. In subclasses both
	 *            retrieveParameters() and addParametersToContext() should be
	 *            overridden.
	 */
	 ...
	 * Override this method to customize the ordering of the parameters as they
	 * are set to the action.
	 *
	...
	/**
	 * Gets a set of regular expressions of parameters to remove from the
	 * parameter map
	 *
