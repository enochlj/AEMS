 ...
 *
 * <p>
 * The evaluation methods check if the value contains "${" before invoking the
 * EL evaluator, treating the value as "normal" expression (i.e. a literal
 * String value) else.
 *
 * <p>
 * <b>See {@link #isSpringJspExpressionSupportActive} for guidelines on when to
 * use Spring's JSP expression support as opposed to the built-in expression
 * support in JSP 2.0+ containers.</b>
 *
...
public abstract class ExpressionEvaluationUtils {
	...
	/**
	 * Expression support parameter at the servlet context level (i.e. a
	 * context-param in <code>web.xml</code>): "springJspExpressionSupport".
	 */
	 ...
	 * However, it will only be active for web applications declaring Servlet
	 * 2.4 or higher in their <code>web.xml</code> deployment descriptor.
	 * <p>
	 * If a <code>web.xml</code> context-param named
	 * "springJspExpressionSupport" is found, its boolean value will be taken to
	 * decide whether this support is active. If not found, the default is for
	 * expression support to be inactive on Servlet 3.0 containers with web
	 * applications declaring Servlet 2.4 or higher in their
	 * <code>web.xml</code>. For backwards compatibility, Spring's expression
	 * support will remain active for applications declaring Servlet 2.3 or
	 * earlier. However, on Servlet 2.4/2.5 containers, we can't find out what
	 * the application has declared, so we'll also fall back to keeping
	 * expression support active in such a case.
	 * <p>
	 * <b>Recommendations:</b> Explicitly set "springJspExpressionSupport" to
	 * "false" in order to prevent double evaluation for Servlet 2.4+ based
	 * applications. On Servlet 3.0 containers, this will be done for you by
	 * default by the framework. If for some reason you nevertheless want
	 * Spring's JSP expression support to be active, explicitly set the
	 * "springJspExpressionSupport" context-param to "true".
	 * 
	 * @param pageContext
	 *            current JSP PageContext
	 * @return <code>true</code> if active (ExpressionEvaluationUtils will
	 *         actually evaluate expressions); <code>false</code> if not active
	 *         (ExpressionEvaluationUtils will return given values as-is,
	 *         relying on the JSP container pre-evaluating values before passing
	 *         them to JSP tag attributes)
	 */
	public static boolean isSpringJspExpressionSupportActive(PageContext pageContext) {
		...
		/*
		 * if (sc.getMajorVersion() >= 3) { // We're on a Servlet 3.0+
		 * container: Let's check what the application declares... if
		 * (sc.getEffectiveMajorVersion() > 2 || sc.getEffectiveMinorVersion() >
		 * 3) { // Application declares Servlet 2.4+ in its web.xml: JSP 2.0
		 * expressions active. // Skip our own expression support in order to
		 * prevent double evaluation. return false; } }
		 */
	...
	/**
	 * Evaluate the given expression (be it EL or a literal String value) to an
	 ...
	 * @param resultClass
	 *            class that the result should have (String, Integer, Boolean)
	 * @param pageContext
	 *            current JSP PageContext
	 * @return the result of the evaluation
	 * @throws JspException
	 *             in case of parsing errors, also in case of type mismatch if
	 *             the passed-in literal value is not an EL expression and not
	 *             assignable to the result class
	 */
	...
	/**
	 * Evaluate the given expression (be it EL or a literal String value) to a
	 * String.
	 * 
	 * @param attrName
	 *            name of the attribute (typically a JSP tag attribute)
	...
	/**
	 * Actually evaluate the given expression (be it EL or a literal String
	 * value) to an Object of a given type. Supports concatenated expressions,
	 ...
	 *            value of the attribute
	 * @param resultClass
	 *            class that the result should have
	 * @param pageContext
	 *            current JSP PageContext
	 * @return the result of the evaluation
	 * @throws JspException
				...
				do {
					...
					if (exprPrefixIndex != -1) {
						...
						if (expr.length() == attrValue.length()) {
							// A single expression without static prefix or
							// suffix ->
							// parse it with the specified result class rather
							// than String.
							return evaluateExpression(attrValue, resultClass, pageContext);
						} else {
							// We actually need to concatenate partial
